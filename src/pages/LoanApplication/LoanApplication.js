...
const TableColumns = [
  ...
  { id: 'loanReviewer', label: 'Loan Reviewer' },
...
function LoanApplication() {
  ...
  const {
    ...
    isLoading: getQueryLoanApplicationIsLoading,
  } = useGetAllLoansQuery();

  const [
    triggerLoanApplications,
    {
      data: lazyQueryLoanApplications,
      isLoading: lazyQueryLoanApplicationsIsLoading,
    },
  ...
  useMemo(() => {
    if (lazyApproveLoanApplicationIsSuccess) {
      ...
      const errorKey = Object.keys(
        lazyApproveLoanApplicationError?.data.errors
      )[0];
      const errorMessage =
        lazyApproveLoanApplicationError?.data.errors[errorKey];
  ...
  useMemo(() => {
    if (lazyDisapproveLoanApplicationIsSuccess) {
      ...
      const errorKey = Object.keys(
        lazyDisapproveLoanApplicationError?.data.errors
      )[0];
      const errorMessage =
        lazyDisapproveLoanApplicationError?.data.errors[errorKey];
    ...
    <Main>
      ...
      {agentDetails && (
        <ViewAgentProfile
          ...
          userId={
            loanModalType === VIEW_AGENT_DETAILS ? agentDetails?.userId : null
          }
          handleClose={() => {
            setLoanModalType('');
      ...
      <Container>
        ...
        <Table
          ...
          loading={
            lazyQueryLoanApplicationsIsLoading ||
            getQueryLoanApplicationIsLoading
          }
